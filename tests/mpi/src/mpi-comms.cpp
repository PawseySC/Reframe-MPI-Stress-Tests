...
#define LOG(ThisTask) std::cout<< _MPI_calling_rank(ThisTask)<< "@" << __func__ << " L"<<__LINE__<<" ("<<getcurtime()<<") : "
    ...
    for (std::string line; std::getline(f, line); )
    {
        ...
        if (start != std::string::npos)
        {
            ...
            //std::cout << "The PID for this process is: " << pid << " and it is running on node " 
                      //<< hostname << std::endl;
            outfile << "The PID for this process is: " << pid << " and it is running on node " 
                    << hostname << std::endl;
            break;
        }
    }
...
void my_MPIAlltoallv(const void* send_buf, const int send_counts[], const int send_offsets[],
                     MPI_Datatype send_type, void* recv_buf, const int recv_counts[], const int recv_offsets[],
                     MPI_Datatype recv_type)
{
    ...
    //               MPI_Comm comm)
    MPI_Alltoallv(send_buf, send_counts, send_offsets, send_type, recv_buf, recv_counts, recv_offsets, 
                  recv_type, MPI_COMM_WORLD);
...
void my_MPIAllgather(const void* send_buf, int send_count, MPI_Datatype send_type, void* recv_buf,
                   ...
                   const int recv_counts[], const int recv_offsets[], MPI_Datatype recv_type, int root)
{
    ...
    MPI_Gatherv(send_buf, send_count, send_type, recv_buf, recv_counts, recv_offsets, recv_type, 0, 
                MPI_COMM_WORLD);
...
void reportTime(std::chrono::time_point<Clock> start, std::chrono::time_point<Clock> end,
                std::string func, std::string line)
{
    ...
    if (world_rank == root_rank)
    {
        ...
        std::cout << "@" << func << ": L" << line << " time stats: ["
                  ...
                  << min << ", Maximum = " << max << ", IQR = " << iqr << ",] (us)" << std::endl;
    }
...
void gatherData(std::vector<Data> &data, int iverbose)
{
    ...
    int* rc = &recv_counts[0];
    int* ro = &recv_offsets[0];
    my_MPIGatherv(data.data(), send_bytes, MPI_BYTE, recv_buffer.data(), rc, ro,
                  MPI_BYTE, 0);
