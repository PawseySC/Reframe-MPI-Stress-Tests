...
void reportProcs() {
    ...
    for (std::string line; std::getline(f, line);) {
        ...
        if (start != std::string::npos) {
            ...
            is >> pid;
            // std::cout << "The PID for this process is: " << pid << " and it is running on node " << hostname << std::endl;
...
void redistributeData(std::vector<Data> &data, int ndata, int iverbose) {
    ...
    my_MPIAllreduce(p1, p2, NProcs, MPI_INTEGER, MPI_MAX);
    ...
    if (iverbose) {
	Log() << "Rank " << ThisTask << ": Has [total_send, total_recv] = " << total_send << ", " << total_recv << std::endl;
    }
...
void gatherData(std::vector<Data> &data, int iverbose) {
    ...
    if (ThisTask == root_rank) {
        ...
        if (iverbose) {
	    Log() << "Rank " << ThisTask << ": Root rank now has " << nlocal << " elements" << std::endl;
        }
    }
...
void redistributeData2(std::vector<Data> &data, int &nlocal, int iverbose) {
    ...
    std::remove_copy_if(data.begin(), data.end(), std::back_inserter(send_buffer),
                        [](const Data &datum) { return datum.rank == ThisTask; });
    // Vector to hold the offsets (relative to the start of `send_buffer`) where each rank's data begins
    std::vector<int> send_offsets(NProcs);
    // Fill `send_offsets` by calculating the
    // cumulative sum of `send_counts`
    int total = 0;
    for (auto i = 1; i < NProcs; i++) {
    ...
    my_MPIAllgather(p1, NProcs, MPI_INT, p2, NProcs, MPI_INT);
...
void scatterData(std::vector<Data> &data, int ndata, int iverbose) {
    ...
    Rank0LocalLogger() << "Scattering data from root rank to all ranks" << std::endl;
    ...
    if (iverbose) {
	Log() << "Rank " << ThisTask << ": Receiving " << nrecv[0] << " from root rank" << std::endl;
    }
...
int main(int argc, char *argv[]) {
    ...
    //  std::vector<char> allhostnames(maxsize * NProcs);
    ...
    //  {
    //      std::cout << "Hostnames: ";
    //      for (auto i = 0; i < maxsize * NProcs; i++)
    //      {
    ...
    if (ThisTask == root_rank) {
        ...
        if (iverbose) {
	    Rank0LocalLogger() << "Root rank generated " << ndata << " elements" << std::endl;
        }
