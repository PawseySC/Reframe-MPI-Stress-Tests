...
sys_memory_stats get_system_memory() {
    ...
    while ((pos = text.find(space_delimiter)) != std::string::npos) {
        if (text.substr(0, pos) != space_delimiter)
...
std::string ReportMemUsage(const std::string &function, const std::string &line_num) {
    std::string report;
    memory_usage mem;
...
std::string ReportMemUsage(const memory_usage &prior_mem_usage, const std::string &function,
...
std::tuple<std::string, memory_usage> GetMemUsage(const std::string &function, const std::string &line_num) {
    ...
    auto append_memory_stats = [&memory_report](const char *name, const memory_stats &stats) {
        memory_report << name << " current/peak: " << memory_amount(stats.current) << " / "
                      << memory_amount(stats.peak);
    };
...
// report usage along with change relative to another sampling of memory
std::tuple<std::string, memory_usage> GetMemUsage(const memory_usage &prior_mem_usage, const std::string &function,
                                                  const std::string &line_num) {
    auto memory_usage = get_memory_usage();
    ...
    std::ostringstream memory_report;
    auto append_memory_stats = [&memory_report](const char *name, const memory_stats &stats) {
        memory_report << name << " current/peak/change : " << memory_amount(stats.current) << " / "
                      << memory_amount(stats.peak) << " / " << memory_amount(stats.change);
    };
    memory_report << "Memory report @ " << function << " L" << line_num << " : ";
    append_memory_stats("VM", memory_usage.vm);
    memory_report << "; ";
...
MPIGetNodeMemUsage(MPI_Comm &comm, const std::string &function, const std::string &line_num) {
    ...
    memory_usage nomem;
    for (auto &s : hostnames)
        memonhost.insert(std::pair<std::string, memory_usage>(s, nomem));
    for (auto i = 0; i < commsize; i++) {
        std::string s;
        for (auto j = 0; j < maxsize; j++)
            s += allhostnames[i * maxsize + j];
...
MPIGetNodeSystemMem(MPI_Comm &comm, const std::string &function, const std::string &line_num) {
    ...
    for (auto &m : memonhost) {
        memory_report << "\tNode : " << m.first << " : ";
        append_memory_stats("Total ", m.second.total);
        memory_report << "; ";
        append_memory_stats("Used  ", m.second.used);
        memory_report << "; ";
        append_memory_stats("Free  ", m.second.free);
        memory_report << "; ";
        append_memory_stats("Shared", m.second.shared);
        memory_report << "; ";
        append_memory_stats("Cache ", m.second.cache);
        memory_report << "; ";
        append_memory_stats("Avail ", m.second.avail);
        memory_report << "; ";
        memory_report << " \n";
        namehosts.push_back(m.first);
        memhosts.push_back(m.second);
...
std::string ReportSystemMem(const std::string &function, const std::string &line_num) {
    std::string report;
...
// report usage along with change relative to another sampling of memory
std::string ReportSystemMem(const sys_memory_stats &prior_mem_usage, const std::string &function,
                            const std::string &line_num) {
    std::string report;
    sys_memory_stats mem;
...
std::tuple<std::string, sys_memory_stats> GetSystemMem(const std::string &function, const std::string &line_num) {
    ...
    memory_report << "Memory report @ " << function << " L" << line_num << " : ";
    append_memory_stats("Total ", sys_mem.total);
    memory_report << "; ";
    append_memory_stats("Used  ", sys_mem.used);
    memory_report << "; ";
    append_memory_stats("Free  ", sys_mem.free);
    memory_report << "; ";
    append_memory_stats("Shared", sys_mem.shared);
    memory_report << "; ";
    append_memory_stats("Cache ", sys_mem.cache);
    memory_report << "; ";
    append_memory_stats("Avail ", sys_mem.avail);
    memory_report << "; ";
    return std::make_tuple(memory_report.str(), sys_mem);
}

// report usage along with change relative to another sampling of memory
std::tuple<std::string, sys_memory_stats> GetSystemMem(const sys_memory_stats &prior_mem_usage,
                                                       const std::string &function, const std::string &line_num) {
    auto sys_mem = get_system_memory();
    std::ostringstream memory_report;
    ...
    memory_report << "Memory report @ " << function << " L" << line_num << " : ";
    append_memory_stats("Total ", sys_mem.total, sys_mem.total - prior_mem_usage.total);
    memory_report << "; ";
    append_memory_stats("Used  ", sys_mem.used, sys_mem.used - prior_mem_usage.used);
    memory_report << "; ";
    append_memory_stats("Free  ", sys_mem.free, sys_mem.free - prior_mem_usage.free);
    memory_report << "; ";
    append_memory_stats("Shared", sys_mem.shared, sys_mem.shared - prior_mem_usage.shared);
    memory_report << "; ";
    append_memory_stats("Cache ", sys_mem.cache, sys_mem.cache - prior_mem_usage.cache);
    memory_report << "; ";
    append_memory_stats("Avail ", sys_mem.avail, sys_mem.avail - prior_mem_usage.avail);
    memory_report << "; ";
    return std::make_tuple(memory_report.str(), sys_mem);
}
} // namespace profiling_util
