...
#endif
#ifdef _OPENMP
#pragma omp parallel default(none) shared(binding_report, hnbuf, ThisTask) private(coremask, clbuf) firstprivate(result)
#endif
    {
...
#pragma omp critical
    ...
    MPI_Allgatherv(p2, binding_report.length(), MPI_CHAR, newbindingreport, recvcounts.data(), offsets.data(), MPI_CHAR,
                   MPI_COMM_WORLD);
